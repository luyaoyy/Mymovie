...
<%
    ...
    if (mid_str!=null){
        ...
        if (row>0){
            ...
            response.sendRedirect("/login/SelectTerrorMovieServlet");
        }
    }
%>
<body>
...
<div id="right">
    <nav id="nav">
        <ul>
            ...
            <li id="now"><a href="/login/SelectTerrorMovieServlet?mclass=恐怖">恐怖</a></li>
            <li><a href="/login/SelectCartoonMovieServlet?mclass=动画">动画</a></li>
            <div id="add"><a href="/login/jsp/upmovie.jsp">添加影片</a></div>
        </ul>
    </nav>
    <div id="out">
        <c:forEach items="${sessionScope.terrormovie}" var="movie">
            <div id="movie_card">
                ...
                <div><div id="edit"><a href="/login/EditMovieGetMovieServlet?id=${movie.id}">编辑</a></div><div id="dele"><a href="/login/jsp/h2terrormovie.jsp?mid=${movie.id}" onclick="return confirm('是否确认将此电影下架?')">下架</a></div></div>
            </div>
        </c:forEach>
    </div>
</div>
